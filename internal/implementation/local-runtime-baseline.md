...
- automated test runtime separation
```
...
- full application integration tests may require Testcontainers when the real application context requires persistence
- DB-free web-layer tests should use an isolated web-layer boundary and should not require Docker, Podman, PostgreSQL, Flyway, or JPA
```

The test runtime is separate from manual dev runtime.

The test runtime does not require the application to support DB-disabled full application startup.
...
- full application tests should disable infrastructure required by the real application context
- DB-free web-layer tests should require container runtime
...
DB-free web-layer tests remain independent from local infrastructure.

Full application integration tests use required infrastructure instead of disabling it.
...
Do not assume the application must support DB-disabled full startup unless that becomes an explicit project requirement.
...
```text
Local runtime planning must preserve the separation between dev runtime, automated DB-backed test runtime, and DB-free isolated web-layer testing while keeping local infrastructure externalized through Compose and Docker-compatible tooling.
```
